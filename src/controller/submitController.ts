...
export default async function submitController(fastify: FastifyInstance) {
  // GET /
  fastify.post("/code/submit", {}, async function (
    ...
    const runCode = await java.runSource(code, { compileTimeout: 10000, stdin: input }).then((result) => {
    ...
    switch (runCode.errorType) {
      case "compile-time":
        reply.status(200).send({ message: "An error occurred during compilation", output: runCode })
        return;
      case "run-time":
        reply.status(200).send({ message: "An error occurred during runtime, recheck your code for possible edge cases.", output: runCode })
        return;
    }
